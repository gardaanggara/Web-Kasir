...
while($row = mysqli_fetch_array($lihat)) {     
        ...
        //Untuk Button
       			  if (isset($_GET['id'])&&isset($_GET['ubah'])) {
        					...
        					if ($_GET['id']==$row['id_transaksi']&&$_GET['ubah']==true) {
        					echo "<td><button class='btn btn-primary' name=simpan>Simpan</button> || <a class='btn btn-danger' name='batal' href='list-transaksi.php'>Batal</a></td>";
		    		
		    				}else{
							echo "<td><a class='btn btn-warning' href='list-transaksi.php?id=$row[id_transaksi]&ubah=true'>Ubah</a></td></tr>";        
